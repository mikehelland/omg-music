...
// this file contains OMGSong, OMGSongPart, and OMusicPlayer
...
OMusicPlayer.prototype._play = function () {
    ...
    if (this.playing) {
        setTimeout(() => {this._play();}, (this.nextBeatTime - this.context.currentTime) * 1000 - this.latency)
    ...
    this.latencyMonitor = Math.round((timeToPlay - this.context.currentTime) * 1000);
    ...
    if (part.synth) {
        ...
        if (audio) {
            audio.bufferGain.gain.linearRampToValueAtTime(part.data.audioParams.gain, 
                this.context.currentTime + fromNow * 0.995);
            audio.bufferGain.gain.linearRampToValueAtTime(0, this.context.currentTime + fromNow);
            audio.stop(this.context.currentTime + fromNow);
        }
    }
};
...
OMusicPlayer.prototype.playLiveNotes = function (notes, part) {
    ...
    if (!notes.autobeat) {
        ...
        if (this.disableAudio) {
            ...
            sectionPart.liveNote = this.soundFontPlayer.queueWaveTable(this.context, this.context.destination
