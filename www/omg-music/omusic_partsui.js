...
function OMGMelodyMaker(canvas, part, player) {
	...
	this.dots = [];
	for (var i = 0; i < 5; i++) {
		this.dots.push([Math.random(), Math.random()]);
	}
} 
...
//	context.fillStyle = "#F4F4F4";
	...
	if (this.welcomeStyle) {
		...
		if (this.animationStarted) {
	        ...
	        var cap = "Connect the Dots!";
	        context.fillText(cap, canvas.width / 2 - context.measureText(cap).width / 2,
	                canvas.height / 2 - 20);
	        
	        //context.fillStyle = "#808080";
	        for (var idot = 0; idot < this.dots.length; idot++) {
	        	context.beginPath();
	        	context.arc(canvas.width * this.dots[idot][0], 
	        				canvas.height * this.dots[idot][1]* 0.9, 10, 0, Math.PI * 2);
	        	context.closePath();
	        	context.stroke();
	        }
