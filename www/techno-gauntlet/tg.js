...
tg.warpButton = document.getElementById("warp-button");
...
tg.warpButton.onclick = function () {
    tg.showWarpFragment();
};
...
tg.showMixFragment = function () {
    ...
    tg.song.sections[0].parts.forEach(function (part) {
        ...
        newDiv.className = "mixer-part";
        newDiv.innerHTML = "a";
        newDiv.onmousedown = function (e) {
            e.target.isTouching = true;
            var percent = (e.layerX - e.target.offsetLeft) / e.target.clientWidth;
            part.data.audioParameters.volume = percent;
            newDiv.drawCanvas(e.target);
        };
        newDiv.onmousemove = function (e) {
            if (e.target.isTouching) {
                var percent = (e.layerX - e.target.offsetLeft) / e.target.clientWidth;
                part.data.audioParameters.volume = percent;
                newDiv.drawCanvas(e.target);
            }
        ...
        newDiv.onmouseup = function (e) {
            e.target.isTouching = false;
        };
        newDiv.drawCanvas = function (newDiv) {
            newDiv.width = newDiv.clientWidth;
            newDiv.height = newDiv.clientHeight;
            var ctx = newDiv.getContext("2d");
            ctx.fillStyle = part.data.audioParameters.mute ? "#880000" : "#008800";
            ctx.fillRect(0, 0, part.data.audioParameters.volume * newDiv.clientWidth, newDiv.height);
            ctx.fillStyle = "white";
            ctx.fillText(part.data.soundSet.name, 10, 10);
        };
        tg.mixFragment.appendChild(newDiv);
        divs.push(newDiv);
    });
...
tg.showWarpFragment = function () {
    if (!tg.warpFragment) {
        tg.warpFragment = document.getElementById("warp-fragment");
    }
    var divs = [];
    tg.warpFragment.innerHTML = "";
    tg.song.sections[0].parts.forEach(function (part) {
        var newDiv = document.createElement("canvas");
        newDiv.className = "mixer-part";
        newDiv.innerHTML = "a";
        newDiv.onmousedown = function (e) {
            e.target.isTouching = true;
            var percent = (e.layerX - e.target.offsetLeft) / e.target.clientWidth * 2;
            part.data.audioParameters.warp = percent;
            newDiv.drawCanvas(e.target);
        };
        newDiv.onmousemove = function (e) {
            if (e.target.isTouching) {
        ...
        newDiv.drawCanvas = function (newDiv) {
            ...
            ctx.fillRect(0, 0, part.data.audioParameters.warp / 2 * newDiv.clientWidth, newDiv.height);
            ctx.fillStyle = "white";
            ctx.fillText(part.data.soundSet.name, 10, 10);
        };
        tg.warpFragment.appendChild(newDiv);
        divs.push(newDiv);
    });
    
    tg.hideDetails();
    tg.warpFragment.style.display = "flex";
